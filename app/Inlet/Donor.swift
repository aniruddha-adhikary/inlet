...
nonisolated enum Donor {
    static func appName(_ appID: String) -> String { AppCatalog.shared.name(appID) }
    ...
    static func graph(from store: BridgeStore) throws -> (persons: [MessagePerson], conversations: [ConversationEntity]) {
        ...
        let paused = AppSettings.paused
        for m in try store.allMessages() where !paused.contains(m.appID) {
            let p = person(for: m)
            persons[p.id] = p
    ...
    nonisolated(unsafe) static let index = CSSearchableIndex(name: (Bundle.main.bundleIdentifier ?? "inlet") + ".index")
    ...
    static func donatePending(from store: BridgeStore) async throws -> Int {
        // People and chats are donated after the messages. The flag survives a quit in between,
        // so the next launch finishes the job instead of leaving Siri with messages but no people.
        let graphIsStale = "graph.stale"
        var donated = 0
        while true {
            let batch = try store.pending(excludingApps: AppSettings.paused)
            if batch.isEmpty {
                if UserDefaults.standard.bool(forKey: graphIsStale) {
                    try await donateGraph(from: store)
                    UserDefaults.standard.set(false, forKey: graphIsStale)
                }
                return donated
            }
            UserDefaults.standard.set(true, forKey: graphIsStale)
