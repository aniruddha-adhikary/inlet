...
nonisolated enum Ingest {
    ...
    static func run(batchJSON: Data, pageHost: String) -> (status: Int, bodyJSON: Data) {
        ...
        let result = run(batch: batch, pageHost: pageHost)
    ...
    static func run(batch: [String: Any], pageHost: String, store: BridgeStore = .default, library: ProfileLibrary = .shared) -> (status: Int, body: [String: Any]) {
        ...
        if AppSettings.isPaused(profile.appID) { return (423, ["error": "paused", "status": "paused"]) }
        ...
        let cutoff = AppSettings.cutoff(profile.appID)
        ...
        do {
            ...
            if verdict.ok, let schema = library.schemas[profile.schema] {
                for raw in items {
                    ...
                    if store.isExcluded(appID: profile.appID, conversationID: record["conversationId"] as? String) {
                    ...
                    let id = entityID(appID: profile.appID, schema: profile.schema, sourceID: sourceID)
                    let result = try store.upsert(id: id, appID: profile.appID, schema: profile.schema, record: record,
