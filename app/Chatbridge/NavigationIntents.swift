...
nonisolated enum BridgeAnswer {
    static func search(_ query: String, via path: String) async -> (entities: [MessageEntity], dialog: IntentDialog) {
        ...
        guard !hits.isEmpty else {
            return ([], "I couldn't find any bridged messages mentioning \(query).")
        }
...
// Siri AI's planner routes "search ... in Chatbridge" through this schema and
// nothing else (App Shortcut phrases are ignored by it). Apple requires the
// schema to open the app; Chatbridge is a windowless menu bar app, so that
...
struct SearchChatsIntent: AppIntent {
    static let title: LocalizedStringResource = "Search Chats"
    static let description = IntentDescription("Searches the messages Chatbridge has bridged from your other apps.")
...
struct ChatbridgeShortcuts: AppShortcutsProvider {
