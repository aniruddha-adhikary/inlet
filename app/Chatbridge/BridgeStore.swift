...
nonisolated final class BridgeStore: @unchecked Sendable {
    ...
    func pending(limit: Int = 500) throws -> [BridgedMessage] {
        try run("SELECT \(Self.columns) FROM entities WHERE schema='messages.message' AND donated_hash IS NOT content_hash LIMIT \(limit)") { self.decode($0) }
    }
